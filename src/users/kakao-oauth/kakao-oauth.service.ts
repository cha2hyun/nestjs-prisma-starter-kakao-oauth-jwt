...
export class KakaoLoginService {
  ...
  async getToken(code: string, redirectUri: string): Promise<KakaoOauthToken> {
    try {
      ...
      const tokenResponse = await new Promise<KakaoOauthToken>(
        (resolve, reject) => {
          this.httpService
            .post(
              url,
  ...
  async getUser(token: string) {
    try {
      ...
      const infoResponse = await new Promise<KakaoV2UserMe>(
        (resolve, reject) => {
          this.httpService
            .get(url, {
              headers: {
  ...
  async validate(token: string): Promise<string> {
    try {
      ...
      const infoResponse = await new Promise<{ id: string }>(
        (resolve, reject) => {
          this.httpService
            .get(url, {
              headers: {
